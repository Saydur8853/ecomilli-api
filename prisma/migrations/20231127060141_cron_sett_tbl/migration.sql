/*
  ...
  - A unique constraint covering the columns `[cronTtitle]` on the table `cron_settings` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `cronTtitle` to the `cron_settings` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `cron_settings` ADD COLUMN `cronTtitle` VARCHAR(150) NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX `cron_settings_cronTtitle_key` ON `cron_settings`(`cronTtitle`);
